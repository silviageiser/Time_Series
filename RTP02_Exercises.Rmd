...
The method is valid for correlated error terms. 

Precondition: Residuals after linear regression display an AR(1) behavior

Steps:
1. Perform OLS - ordinary least square fit on data.
...
SARIMA models are non-stationary ARMA models with a Seasonality, which will be removed by differencing with the (I = Integration) and aim to describe the autocorrelations in the data. Seasonal ARIMA model
```{r, echo = F, out.width= '50%'}
